...
The unit test process takes a number of command-line arguments (or `NSUserDefaults` keys) to configure which tests are run, etc.

Example: `$ ./RunTests.sh -TestUserName SpotifyUser -TestPassword password123 -AppKey sljhv445v -KeepExistingCache NO -RunStandardTests YES -RunStressTests YES`

**Note:** It's typically easier to run the tests from Xcode. Switch to the `CocoaLSTests` scheme then add your arguments to the *Arguments* tab of the *Run* section.

- `TestUserName` (string, required): The Spotify user to log in with.

- `TestPassword` (string, required): The password of the Spotify user to log in with.

- `AppKey` (string, required): A base-64 encoded string of your Spotify app key. See above for generation instructions.

- `LogForTeamCity` (bool, optional, default=`NO`): A boolean to tell the unit tests to log the tests in a format recognised by JetBrains TeamCity. Acceptable values: `YES`, `NO`.

- `KeepExistingCache` (bool, optional, default=`NO`): A boolean to tell the unit tests to keep the cache generated by previous test run(s). Pass `NO` to have the tests delete any existing caches for this run. Acceptable values: `YES`, `NO`.

- `RunStandardTests` (bool, optional, default=`YES` if no tests are explicitly asked to run, otherwise `NO`): If set to `YES`, the standard set of tests will be run.

- `RunHelperTests` (bool, optional, default=`YES` if no tests are explicitly asked to run, otherwise `NO`): If set to `YES`, the helper functionality tests will be run.

- `RunStressTests` (bool, optional, default=`NO`): If set to `YES`, the stress tests will be run. **Note:** These tests take a while to run and will use a lot of RAM.

- `RunAllTests` (bool, optional, default=`NO`): If set to `YES`, all tests suites will be run.
